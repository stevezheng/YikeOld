...
var Schema = function(name, options) {
  ...
  newSchema.prototype.save = function(callback) {
    ...
    if (!this.id) {
      for (var fieldName in newSchema.$fields) {
        if (this.shadowObj.get(fieldName) === undefined) {
          var field = newSchema.$fields[fieldName];
          var defaultValue = field.defaultValue();
          if (defaultValue !== undefined) {
            this.shadowObj.set(fieldName, defaultValue);
          }
        }
      }
    }
    ...
    async.waterfall([
        function(next) {
          var f = null;
          newSchema.$uniques.forEach(function(fieldName) {
            f = q.or(f, Q.eq(fieldName, self.shadowObj.get(fieldName)));
          });
          if (self.id) {
            f = q.and(f, Q.not(self.id));
          }
          Q.select(newSchema)
            .where(f)
            .execute(function(err, results) {
              var errors = [];
              newSchema.$uniques.forEach(function(fieldName) {
                var fieldValue = self.shadowObj.get(fieldName);
                results.forEach(function(result) {
                  if (result.get(fieldName) === fieldValue) {
                    errors.push('Duplicate ' + newSchema.$table + '.' + fieldName + ' ' + fieldValue);
                  }
                });
              });
              if (errors.length > 0) {
  ...
  newSchema.prototype.unset = function(fieldName) {
    this.shadowObj.unset(fieldName);
  };

  newSchema.prototype.toJSON = function() {
    return this.shadowObj.toJSON();
  };
...
Select.prototype.where = function(condition) {
  if (condition.toCQL) {
    ...
    var parseQuery = function($andOr, originKey, value) {
      var op = 'eq';
      var key = originKey;
      ...
      f = Q[$andOr.substr(1)](f, Q[op](key, condition[originKey]));
    };

    for (var key in condition) {
      if (key === '$or' || key === '$and') {
        for (var k1 in condition[key]) {
          parseQuery(key, k1, condition[key][k1]);
        }
      } else {
        parseQuery('$and', key, condition[key]);
      }
    }
    this.$condition = f;
  }
...
Select.prototype.execute = function(callback) {
  ...
  var args = [];
  if (this.$condition) {
    var where = this.$condition.toCQL();
    if (where) {
      cql.push('where')
      cql.push(where);
  ...
  var self = this;
  console.log("Query>>> CQL: ", cql.join(' '), '| Args: ', args);
